...
df_top250 = pd.read_csv("source/Top250.csv")

print("\n First 5 Rows of Dataset:")
print(df_top250.head())

# Data Cleaning & Preprocessing
df_top250["Sales"] = pd.to_numeric(df_top250["Sales"], errors="coerce")  # Convert sales to float
df_top250["YOY_Sales"] = df_top250["YOY_Sales"].str.rstrip('%').astype(float)  # Convert % to float
df_top250["Units"] = pd.to_numeric(df_top250["Units"], errors="coerce")  # Convert to int safely
df_top250["YOY_Units"] = df_top250["YOY_Units"].str.rstrip('%').astype(float)  # Convert % to float

# Handle missing values (replace instead of dropping all data)
df_top250.fillna({"YOY_Sales": df_top250["YOY_Sales"].median(),
                  "YOY_Units": df_top250["YOY_Units"].median(),
                  "Sales": df_top250["Sales"].median(),
                  "Units": df_top250["Units"].median()}, inplace=True)

# Exploratory Data Analysis (EDA)
# **Sales Distribution**
plt.figure(figsize=(10, 6))
sns.histplot(df_top250["Sales"], bins=20, kde=True)
